...
export default function Sidebar({ activePanel, onPanelChange }: Props) {
  const tt = useT()
  ...
  return (
    <nav className="sidebar no-select">
      {panels.map(p => (
        <button key={p.id} className={`sidebar-btn ${cur === p.id ? 'active' : ''}`}
          onClick={() => onPanelChange(p.id)} aria-label={tt(p.label)}>
